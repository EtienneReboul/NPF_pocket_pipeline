#!/usr/bin/env python3
"""
scripts/generate_msa_clustering_blosum62_umap_hdbscan_all_proteins_notebook.py
================================================================================
Builds a single notebook, `notebook/msa_clustering/all_proteins_blosum62_umap_hdbscan.ipynb`,
that batch-processes every NPF protein under `data/msa/a3m/` with a different
embedding + clustering combination than the other MSA-clustering notebooks in
this repo: sequences are encoded with a BLOSUM62-like numeric embedding (adapted
from an external `cluster-msa.py` reference script's `encode_seqs_bl62` /
`BL62NP` — each residue mapped to a 16-dim substitution-based vector) instead
of one-hot encoding, embedded with UMAP to `UMAP_N_COMPONENTS` dimensions (5 by default,
following the "On the Validation of HDBSCAN" blog post's recommendation that
density-based clustering benefits from a modest-dimensional embedding rather
than a 2D one used purely for plotting), and clustered with HDBSCAN whose
hyperparameters (`min_samples`, `min_cluster_size`, `cluster_selection_method`,
`metric`) are tuned per protein via a random search scored by DBCV
(Density-Based Clustering Validation, Moulavi et al. 2014) — unlike Silhouette
Score, DBCV accounts for noise and measures density rather than distance,
which actually matches a density-based algorithm's assumptions.

Note on the hyperparameter search: the blog post drives the search with
sklearn's `RandomizedSearchCV`, which by default cross-validates by splitting
rows into folds. That doesn't make sense here — folding a single MSA's
embedding fragments its density structure before HDBSCAN ever sees the whole
picture. Instead, each sampled hyperparameter combination is fit once on the
*entire* per-protein embedding and scored with `hdbscan.validity.validity_index`
directly; the combination with the highest DBCV is refit and used for the
final clustering. Combinations that collapse to fewer than 2 clusters, or that
error out inside DBCV, are scored as the worst possible so they're never
selected.

Per-cluster consensus + `.a3m` subsets and uniform-control MSAs are written
out, plus a gradient-colored UMAP landscape plot (first two of the
`UMAP_N_COMPONENTS` dimensions), a DBCV-vs-hyperparameter-combination
diagnostic plot, and a cross-protein summary table.

Usage:
    python scripts/generate_msa_clustering_blosum62_umap_hdbscan_all_proteins_notebook.py
...
NOTEBOOK_PATH = ROOT / "notebook" / "msa_clustering" / "all_proteins_blosum62_umap_hdbscan.ipynb"
...
def build_notebook() -> dict:
    cells = [
        md(
            "# MSA Clustering — All Proteins (BLOSUM62 + UMAP + DBCV-tuned HDBSCAN)",
            "",
            "Different embedding/clustering combination from the other MSA-clustering "
            "notebooks in this repo: sequences are encoded with a **BLOSUM62-like "
            "numeric embedding** (adapted from an external `cluster-msa.py` reference "
            "script's `encode_seqs_bl62` / `BL62NP` — each residue mapped to a 16-dim "
            "substitution-based vector) instead of one-hot encoding, embedded with "
            "**UMAP** to "
            "`UMAP_N_COMPONENTS` dimensions (5 by default — density-based clustering "
            "benefits from a modest-dimensional embedding, not just the 2D one used "
            "for plotting elsewhere in this repo), and clustered with **HDBSCAN** "
            "whose `min_samples` / `min_cluster_size` / `cluster_selection_method` / "
            "`metric` are tuned per protein via a random hyperparameter search scored "
            "by **DBCV** (Density-Based Clustering Validation, Moulavi et al. 2014) — "
            "unlike Silhouette Score, DBCV accounts for noise and measures density "
            "rather than distance, matching a density-based algorithm's assumptions "
            "(see \"On the Validation of HDBSCAN\").",
            "",
            ...
            "are written out, along with a gradient-colored UMAP landscape plot (first "
            "two of the `UMAP_N_COMPONENTS` dimensions), a DBCV-vs-hyperparameter-combo "
            "diagnostic plot, and a cross-protein summary table.",
            "",
            "Reads every `data/msa/a3m/*.a3m` file (override via the `PROTEINS` "
            "parameter below to run a subset). Outputs go to "
            "`results/msa_clustering/blosum62_umap_hdbscan/<protein>/`, its own "
            "folder so they don't collide with the one-hot-encoded notebooks' "
            "outputs.",
            "",
            "**Runtime note:** each protein fits HDBSCAN + computes DBCV up to "
            "`N_HYPERPARAM_SEARCH_ITER` times (default 60); DBCV's internal pairwise "
        ...
        code(
            ...
            'RESULTS_DIR = ROOT / "results" / "msa_clustering" / "blosum62_umap_hdbscan"',
            "",
            ...
            "UMAP_N_COMPONENTS = 5        # dimensionality HDBSCAN clusters on (blog convention: more than 2D)",
            "UMAP_N_NEIGHBORS = 15",
            "UMAP_MIN_DIST = 0.0          # tighter packing (vs. the OHE notebook's 0.1) helps density-based clustering",
            'UMAP_METRIC = "euclidean"    # BL62 vectors are continuous, not binary indicators, so unlike the OHE',
            "                             # notebook's \"jaccard\" metric, Euclidean is the appropriate choice here",
        ...
        md(
            ...
            "",
            "BLOSUM62-encode → UMAP embed (`UMAP_N_COMPONENTS`-D) → DBCV-tuned HDBSCAN "
            "on the full-dimensional embedding → per-cluster consensus/`.a3m` + "
            "uniform controls + landscape plot (first two UMAP dimensions). Returns a "
        ...
        code(
            ...
            "    umap_reducer = umap.UMAP(n_components=UMAP_N_COMPONENTS, n_neighbors=UMAP_N_NEIGHBORS,",
            "                              min_dist=UMAP_MIN_DIST, metric=UMAP_METRIC, random_state=RANDOM_STATE)",
            "    umap_embedding = umap_reducer.fit_transform(bl62_enc)",
            "    umap_query_embedding = umap_reducer.transform(query_enc)",
            "    umap_cols = [f\"UMAP {i + 1}\" for i in range(UMAP_N_COMPONENTS)]",
            "    for i, col in enumerate(umap_cols):",
            "        df[col] = umap_embedding[:, i]",
            "        query[col] = umap_query_embedding[:, i]",
            "",
            "    hdbscan_labels, best = select_hdbscan_hyperparams_and_cluster(",
            "        df[umap_cols].values, HDBSCAN_MIN_SAMPLES_CANDIDATES, HDBSCAN_MIN_CLUSTER_SIZE_CANDIDATES,",
            ...
            "",
            '    plot_landscape("UMAP 1", "UMAP 2", df, query.iloc[0], "cluster_label_hdbscan",',
            '                   f"{protein} — BLOSUM62+UMAP+HDBSCAN (DBCV={best[\'dbcv\']:.2f})",',
            '                   out_dir / f"{protein}_UMAP_HDBSCAN.pdf")',
            "",
            "    return {",
            '        "protein": protein,',
            '        "n_seqs": n_after,',
        ...
        code(
            ...
            'summary_outfile = RESULTS_DIR / "all_proteins_blosum62_umap_hdbscan_summary.tsv"',
