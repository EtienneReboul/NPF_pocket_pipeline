#!/usr/bin/env python3
"""
scripts/generate_msa_clustering_blosum62_pca_hdbscan_all_proteins_notebook.py
================================================================================
Builds a single notebook, `notebook/msa_clustering/all_proteins_blosum62_pca_hdbscan.ipynb`,
...
swapping the embedding step for **PCA** instead of UMAP: sequences are encoded
with a BLOSUM62-like numeric embedding (adapted from an external
...
encoding, embedded with `sklearn.decomposition.PCA` to `PCA_N_COMPONENTS`
dimensions (5 by default, same modest-dimensional convention as the UMAP
notebook, so density-based clustering isn't restricted to a 2D projection),
and clustered with HDBSCAN whose hyperparameters (`min_samples`,
...
Note on PCA vs. UMAP: PCA is a linear, variance-maximizing projection with an
exact, deterministic `.transform()` for new points (like UMAP, and unlike
t-SNE, so the query sequence is embedded the same way as the rest of the
notebooks rather than needing to be concatenated in before fitting). Unlike
UMAP it has no neighborhood-graph hyperparameters to tune — the only knob is
`PCA_N_COMPONENTS` — but it also can't unfold nonlinear structure the way UMAP
can, so it's included here as a linear baseline against the UMAP and t-SNE
notebooks' nonlinear embeddings. The variance explained by *each* of the
`PCA_N_COMPONENTS` components (not just their sum) is recorded per protein —
as a `{protein}_pca_explained_variance.tsv` table and a scree plot — so it's
possible to tell whether the chosen `PCA_N_COMPONENTS` is actually capturing
most of the encoding's structure or whether a low cumulative value means the
BLOSUM62 encoding's structure is more nonlinear than PCA can capture.
...
out, plus a gradient-colored PCA landscape plot (first two of the
`PCA_N_COMPONENTS` dimensions), a per-component PCA explained-variance scree
plot, a DBCV-vs-hyperparameter-combination diagnostic plot, and a
cross-protein summary table.

Usage:
    python scripts/generate_msa_clustering_blosum62_pca_hdbscan_all_proteins_notebook.py
...
DBSCAN-only notebooks. PCA itself is much cheaper than UMAP or t-SNE, so this
notebook's runtime is dominated almost entirely by the HDBSCAN/DBCV search.
"""
...
NOTEBOOK_PATH = ROOT / "notebook" / "msa_clustering" / "all_proteins_blosum62_pca_hdbscan.ipynb"
...
def build_notebook() -> dict:
    cells = [
        md(
            "# MSA Clustering — All Proteins (BLOSUM62 + PCA + DBCV-tuned HDBSCAN)",
            "",
            "Same BLOSUM62 + DBCV-tuned HDBSCAN combination as "
            "`all_proteins_blosum62_umap_hdbscan.ipynb`, but with **PCA** in place "
            ...
            "one-hot encoding, embedded with `sklearn.decomposition.PCA` to "
            "`PCA_N_COMPONENTS` dimensions (5 by default — same modest-dimensional "
            "convention as the UMAP notebook, so HDBSCAN isn't restricted to a 2D "
            "projection), and clustered with **HDBSCAN** whose `min_samples` / "
            "`min_cluster_size` / `cluster_selection_method` / `metric` are tuned "
            "per protein via a random hyperparameter search scored by **DBCV** "
            "(Density-Based Clustering Validation, Moulavi et al. 2014) — unlike "
            "Silhouette Score, DBCV accounts for noise and measures density rather "
            "than distance, matching a density-based algorithm's assumptions (see "
            "\"On the Validation of HDBSCAN\").",
            "",
            "**PCA vs. UMAP:** PCA is a linear, variance-maximizing projection with "
            "an exact, deterministic `.transform()` for new points (like UMAP, and "
            "unlike t-SNE — so the query sequence is embedded directly rather than "
            "needing to be concatenated in before fitting). It has no "
            "neighborhood-graph hyperparameters to tune — the only knob is "
            "`PCA_N_COMPONENTS` — but it also can't unfold nonlinear structure the "
            "way UMAP can, so it serves here as a linear baseline against the UMAP "
            "and t-SNE notebooks' nonlinear embeddings. The variance explained by "
            "**each** of the `PCA_N_COMPONENTS` components (not just their sum) is "
            "recorded per protein — as a `{protein}_pca_explained_variance.tsv` "
            "table, a scree plot, and per-component columns in the cross-protein "
            "summary — so it's possible to tell whether the chosen "
            "`PCA_N_COMPONENTS` is actually capturing most of the encoding's "
            "structure, rather than just seeing a single cumulative number.",
            "",
            ...
            "are written out, along with a gradient-colored PCA landscape plot (first "
            "two of the `PCA_N_COMPONENTS` dimensions), a per-component PCA "
            "explained-variance scree plot, a DBCV-vs-hyperparameter-combo "
            "diagnostic plot, and a cross-protein summary table.",
            "",
            "Reads every `data/msa/a3m/*.a3m` file (override via the `PROTEINS` "
            "parameter below to run a subset). Outputs go to "
            "`results/msa_clustering/blosum62_pca_hdbscan/<protein>/`, its own "
            ...
            "notebooks. PCA itself is much cheaper than UMAP or t-SNE, so this "
            "notebook's runtime is dominated almost entirely by the HDBSCAN/DBCV "
            "search.",
        ),
        code(
            "import numpy as np",
        ...
        code(
            ...
            'RESULTS_DIR = ROOT / "results" / "msa_clustering" / "blosum62_pca_hdbscan"',
            "",
            ...
            "PCA_N_COMPONENTS = 5         # dimensionality HDBSCAN clusters on (same convention as the UMAP notebook)",
        ...
        code(
            ...
            "",
            "",
            "def record_pca_variance(pca, out_dir, protein):",
            '    """Per-component + cumulative explained variance ratio for a fitted PCA: written to a',
            "    TSV (one row per component) and plotted as a bar+line scree plot, so a low value can be",
            '    read directly as a sign the BLOSUM62 encoding\'s structure is more nonlinear than PCA can capture."""',
            ...
            "    })",
            '    variance_df.to_csv(out_dir / f"{protein}_pca_explained_variance.tsv", index=False, sep="\\t")',
            "",
            "    fig, ax = plt.subplots(figsize=(5, 4))",
            '    ax.bar(variance_df["component"], variance_df["variance_ratio"], color="steelblue",',
            '           label="per-component")',
            '    ax.plot(variance_df["component"], variance_df["cumulative_variance_ratio"], color="red",',
            '            marker="o", label="cumulative")',
            '    ax.set_xlabel("PCA component")',
            '    ax.set_ylabel("fraction of variance explained")',
            '    ax.set_xticks(variance_df["component"])',
            '    ax.set_title(f"{protein} — PCA explained variance")',
            "    ax.legend()",
            "    plt.tight_layout()",
            '    plt.savefig(out_dir / f"{protein}_pca_explained_variance.pdf", bbox_inches="tight")',
            "    plt.close(fig)",
            "",
            "    return ratios",
        ),
        md(
            ...
            "",
            "BLOSUM62-encode → PCA embed (`PCA_N_COMPONENTS`-D, explained variance recorded "
            "per component) → DBCV-tuned HDBSCAN "
            "on the full-dimensional embedding → per-cluster consensus/`.a3m` + "
            "uniform controls + landscape plot (first two PCA dimensions). Returns a "
            "one-row summary dict used to build the cross-protein table in the last "
            "section.",
        ),
        code(
            ...
            "    n_after = len(df)",
            "",
            "    bl62_enc = encode_seqs_bl62(df.sequence.tolist(), max_len=L)",
            "    query_enc = encode_seqs_bl62(query.sequence.tolist(), max_len=L)",
            "    pca_reducer = PCA(n_components=PCA_N_COMPONENTS, random_state=RANDOM_STATE)",
            "    pca_embedding = pca_reducer.fit_transform(bl62_enc)",
            "    pca_query_embedding = pca_reducer.transform(query_enc)",
            "    pca_variance_ratios = record_pca_variance(pca_reducer, out_dir, protein)",
            "    pca_explained_variance = float(pca_variance_ratios.sum())",
            "    pca_cols = [f\"PCA {i + 1}\" for i in range(PCA_N_COMPONENTS)]",
            "    for i, col in enumerate(pca_cols):",
            "        df[col] = pca_embedding[:, i]",
            "        query[col] = pca_query_embedding[:, i]",
            "",
            "    hdbscan_labels, best = select_hdbscan_hyperparams_and_cluster(",
            "        df[pca_cols].values, HDBSCAN_MIN_SAMPLES_CANDIDATES, HDBSCAN_MIN_CLUSTER_SIZE_CANDIDATES,",
            ...
            "",
            '    plot_landscape("PCA 1", "PCA 2", df, query.iloc[0], "cluster_label_hdbscan",',
            '                   f"{protein} — BLOSUM62+PCA+HDBSCAN (DBCV={best[\'dbcv\']:.2f})",',
            '                   out_dir / f"{protein}_PCA_HDBSCAN.pdf")',
            "",
            "    summary = {",
            '        "protein": protein,',
            '        "n_seqs": n_after,',
            '        "L": L,',
            '        "pca_explained_variance": round(pca_explained_variance, 4),',
            ...
            '        summary[f"pca_var_pc{i + 1}"] = round(float(ratio), 4)',
            "    return summary",
        ),
        md(
        ...
        code(
            ...
            "        per_pc = ', '.join(f\"PC{i + 1}={summary[f'pca_var_pc{i + 1}']:.1%}\"",
            "                            for i in range(PCA_N_COMPONENTS))",
            '        print(f"  PCA: {per_pc} -> {summary[\'pca_explained_variance\']:.1%} cumulative")',
        ...
        code(
            ...
            'summary_outfile = RESULTS_DIR / "all_proteins_blosum62_pca_hdbscan_summary.tsv"',
