#!/usr/bin/env python3
"""
scripts/generate_msa_clustering_tsne_dbscan_gmm_all_proteins_notebook.py
===========================================================================
Builds a single notebook, `notebook/msa_clustering/all_proteins_tsne_dbscan_gmm.ipynb`,
that batch-processes every NPF protein under `data/msa/a3m/` combining the two
existing t-SNE clustering notebooks: DBSCAN is run first on the t-SNE
embedding purely to strip noise (the `-1` label), then a Gaussian Mixture
Model is swept on the remaining (denoised) points only, with n_components
ranging from `MIN_COMPONENTS` to `n_dbscan_clusters + GMM_EXTRA_COMPONENTS`
(the number of non-noise DBSCAN clusters, plus a fixed headroom). The number
of components is picked from the knee of the BIC curve via KneeLocator
(falling back to the global BIC minimum if no knee is found), same convention
as `scripts/gmm_conformation.py` and the standalone GMM notebook. Sequences
DBSCAN calls noise keep a `-1` label straight through to the final clustering
(they are never assigned to a GMM component). Per-cluster consensus + `.a3m`
subsets and uniform-control MSAs are written out, plus a gradient-colored
t-SNE landscape plot and a cross-protein summary table.

Usage:
    python scripts/generate_msa_clustering_tsne_dbscan_gmm_all_proteins_notebook.py
"""
...
NOTEBOOK_PATH = ROOT / "notebook" / "msa_clustering" / "all_proteins_tsne_dbscan_gmm.ipynb"
...
def build_notebook() -> dict:
    cells = [
        md(
            "# MSA Clustering — All Proteins (t-SNE + DBSCAN-denoised GMM)",
            "",
            "Batch version of the t-SNE MSA-clustering workflow combining "
            "`all_proteins_tsne_dbscan.ipynb` and `all_proteins_tsne_gmm.ipynb`: for each "
            "protein, DBSCAN is run on the t-SNE embedding first (eps auto-selected via a "
            "k-distance Kneedle knee) purely to strip noise, then a **Gaussian Mixture "
            "Model** is swept only on the DBSCAN-denoised points, with `n_components` "
            "ranging from `MIN_COMPONENTS` to `n_dbscan_clusters + GMM_EXTRA_COMPONENTS` "
            "(the number of non-noise DBSCAN clusters found for that protein, plus a fixed "
            "headroom). BIC is recorded at each step and the number of components is chosen "
            "from the **knee of the BIC curve** via `KneeLocator` (falling back to the "
            "global BIC minimum if no knee is found — the same convention used in "
            ...
            "written out, along with a gradient-colored t-SNE landscape plot and a "
            ...
            "`results/msa_clustering/tsne_dbscan_gmm/<protein>/`, its own folder so they "
            "don't collide with the plain DBSCAN or plain GMM notebooks' outputs.",
        ),
        code(
            "import numpy as np",
            "import pandas as pd",
        ...
        code(
            ...
            'RESULTS_DIR = ROOT / "results" / "msa_clustering" / "tsne_dbscan_gmm"',
            "",
        ...
        md(
            ...
            "",
            "One-hot encode → t-SNE embed → Kneedle-selected DBSCAN (noise removal only) → "
        ...
        code(
            ...
            '        df[["TSNE 1", "TSNE 2"]].values, MIN_SAMPLES, EPS_VAL, "dbscan", out_dir, protein',
            "    )",
            ...
            '            df_clean[["TSNE 1", "TSNE 2"]].values, MIN_COMPONENTS, max_components,',
            ...
            "",
            '    plot_landscape("TSNE 1", "TSNE 2", df, query.iloc[0], "cluster_label_dbscan_gmm",',
            '                   f"{protein} — DBSCAN-denoised GMM (k={best_k})",',
            '                   out_dir / f"{protein}_TSNE_DBSCAN_GMM.pdf")',
            "",
            "    return {",
            '        "protein": protein,',
            '        "n_seqs": n_after,',
        ...
        code(
            ...
            'summary_outfile = RESULTS_DIR / "all_proteins_tsne_dbscan_gmm_summary.tsv"',
